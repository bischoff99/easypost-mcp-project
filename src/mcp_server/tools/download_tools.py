...
def download_file(url: str, filepath: Path) -> bool:
    ...
    """
    try:
        response = requests.get(url, timeout=BULK_OPERATION_TIMEOUT)
...
def register_download_tools(mcp, easypost_service=None):
    ...
    @mcp.tool(
        tags=["download", "labels", "customs", "shipping"],
        annotations={
            "readOnlyHint": True,  # Downloads documents only, no modifications
        },
    )
