...
def mark_preferred_rates(
    ...
    """
    Mark preferred rates based on carrier preference.
    ...
    return [
        {**rate, "preferred": is_preferred_carrier(rate.get("carrier", ""), preferred_upper)}
        for rate in rates
    ]
...
def select_best_rate(
    ...
    """
    Select best rate (preferred carrier if available, otherwise cheapest).
    ...
    if purchase_labels:
        # Try preferred first
        preferred = next((r for r in marked_rates if r.get("preferred")), None)
        if preferred:
            return preferred
